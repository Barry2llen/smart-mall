...
public class CouponHistoryController {
    ...
    @Parameters({
            ...
            @Parameter(name = "useType", description = "使用状态")
    })
    @Operation(summary = "获取CouponHistory列表")
    @GetMapping("/list")
    public R<List<CouponHistory>> getCouponHistorys(@RequestParam Integer pageNum, @RequestParam Integer pageSize,
                                                    @RequestParam(required = false) String memberNickName,
                                                    @RequestParam(required = false) Integer useType) {
        return R.success(couponHistoryService.list(pageNum, pageSize, memberNickName, useType));
    }
