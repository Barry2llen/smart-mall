...
public class MemberPriceServiceImpl extends ServiceImpl<MemberPriceMapper, MemberPrice> implements MemberPriceService {
    ...
    @Override
    public List<MemberPrice> list(Integer pageNum, Integer pageSize) {
        IPage<MemberPrice> page = new Page<>(pageNum, pageSize);
        LambdaQueryWrapper<MemberPrice> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.orderByDesc(MemberPrice::getId);
