...
public class OSSController {
    ...
    //限定上传到OSS的文件前缀。
    //指定过期时间，单位为秒。
    private final Long expire_time = 120L;
    private final List<Object> initConditions;

    public OSSController(OSSProperties ossProperties) throws Exception {
        //OSS基础信息 替换为实际的 bucket 名称、 region-id、host。
        region = ossProperties.getOss().getRegion();
        host = ossProperties.getOss().getHost();
        endpoint = ossProperties.getOss().getEndpoint();
        accessKey = ossProperties.getAccessKey();
        secretKey = ossProperties.getSecretKey();
        arn = ossProperties.getArn();

        initConditions = new ArrayList<>();
        Map<String, String> bucketCondition = new HashMap<>(1);
        bucketCondition.put("bucket", ossProperties.getOss().getBucket());
        initConditions.add(bucketCondition);

        Map<String, String> signatureVersionCondition = new HashMap<>(1);
        signatureVersionCondition.put("x-oss-signature-version", "OSS4-HMAC-SHA256");
        initConditions.add(signatureVersionCondition);

        initConditions.add(Arrays.asList("content-length-range", 1, 10240000));
        initConditions.add(Arrays.asList("eq", "$success_action_status", "200"));
    }
    ...
    public AssumeRoleResponseBody.AssumeRoleResponseBodyCredentials getCredential() throws Exception {
        ...
        com.aliyun.sts20150401.models.AssumeRoleRequest assumeRoleRequest = new com.aliyun.sts20150401.models.AssumeRoleRequest()
                ...
                .setRoleArn(arn)
                .setRoleSessionName("smart-mall-file-upload");// 自定义会话名称
    ...
    public ResponseEntity<Map<String, String>> getPostSignatureForOssUpload(@RequestParam String callback,
        ...
        // 步骤1：创建policy。
        String x_oss_credential = sts_data.accessKeyId + "/" + date + "/" + region + "/oss/aliyun_v4_request";
        ...
        List<Object> conditions = new ArrayList<>(initConditions);

        Map<String, String> securityTokenCondition = new HashMap<>(1);
        securityTokenCondition.put("x-oss-security-token", sts_data.securityToken);
        ...
        conditions.add(Arrays.asList("starts-with", "$key", getUploadDir()));
        ...
        // 步骤3：计算SigningKey。
        byte[] dateKey = hmacsha256(("aliyun_v4" + sts_data.accessKeySecret).getBytes(), date);
        ...
        jsonCallback.put("callbackBody", CallbackBody.callbackBody(callbackVars));
        ...
        //String base64CallbackVar = BinaryUtil.toBase64String(CallbackBody.callbackVar(callbackVars).getBytes());
        ...
        response.put("dir", getUploadDir());
        response.put("host", host);
        response.put("callback", base64CallbackBody);
        response.put("callback-var", CallbackBody.callbackVar(callbackVars));
