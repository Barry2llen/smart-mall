package edu.nchu.mall.services.product.dao;

import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import edu.nchu.mall.components.utils.Entry;
import edu.nchu.mall.models.entity.SkuSaleAttrValue;
import edu.nchu.mall.models.vo.SkuItemSaleAttrVO;
import edu.nchu.mall.services.product.vo.SaleAttrValueVO;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.Collection;
import java.util.List;

@Mapper
public interface SkuSaleAttrValueMapper extends BaseMapper<SkuSaleAttrValue> {
    List<SkuItemSaleAttrVO> getSaleAttrsBySpuId(@Param("spuId") Long spuId);

    List<String> getSaleAttrValues(@Param("skuId") Long skuId);

    List<SaleAttrValueVO> getBatchSaleAttrValues(@Param("skuIds") Collection<Long> skuIds);

    List<Entry<String, Long>> getSkuAttrMappingBySpuId(@Param("spuId") Long spuId);
}
